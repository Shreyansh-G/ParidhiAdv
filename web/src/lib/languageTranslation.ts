...
declare global {
  interface Window {
    /** Injected by the Google Translate widget script, if it ever loads. */
...
export function isGoogleTranslateAvailable(): boolean {
  // `!= null` (not `!== null`): when the widget hasn't loaded, `google?.translate`
  // is *undefined*, and `undefined !== null` is true — so the old check reported
  // "available" in every browser, always.
  return typeof window !== 'undefined' && window.google?.translate != null
}
