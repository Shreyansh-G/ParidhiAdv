// Project intelligence for Paridhi.
//
// Primary path: Claude-powered `generateProjectInsight` Cloud Function
// (structured outputs, cached per project in Firestore + localStorage).
// Fallback path: the local keyword heuristics below, so the UI always
// renders something instantly and works offline / signed-out.

import { httpsCallable } from 'firebase/functions'
import { firebaseFunctions } from './firebase'
import staticInsights from '../data/projectInsights.json'
import { insightCache } from './storage'
...
/**
 * Fetch the real AI insight for a project (Claude via Cloud Functions).
 * Returns null when unavailable (offline, signed out, or backend not
 * deployed yet) — callers should fall back to the local heuristics.
 */
export async function getProjectInsight(projectId: string): Promise<ProjectInsight | null> {
  // 0. Static insights shipped with the app (authored at dev time — works in
  //    production with zero runtime AI cost, online or offline).
  const bundled = (staticInsights as Record<string, ProjectInsight>)[projectId]
  if (bundled) return bundled

  // 1. Device cache (TTL + LRU eviction handled by ./storage).
  const cached = insightCache.get(projectId) as ProjectInsight | null
  if (cached) return cached

  if (!firebaseFunctions) return null

  try {
    const call = httpsCallable<{ projectId: string }, { insight: ProjectInsight }>(
      firebaseFunctions,
      'generateProjectInsight',
    )
    const result = await call({ projectId })
    const insight = result.data?.insight ?? null
    if (insight) insightCache.set(projectId, insight)
    return insight
  } catch (error) {
    console.warn('AI insight unavailable, falling back to local heuristics', error)
    return null
  }
}

// ---------------------------------------------------------------------------
// Local heuristic fallbacks (no network) — used when the AI backend is
// unreachable, and as instant placeholders while the real insight loads.
// ---------------------------------------------------------------------------
