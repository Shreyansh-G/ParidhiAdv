...
import { DEMO_CONTENT_ENABLED, demoNotifications } from '../data/demoContent';
...
export default function NotificationsPage() {
  ...
  // Results are tagged with the (user, filter) they answer, so `notifications`
  // and `loading` are both derived. Switching filters can then never show the
  // previous filter's rows, and a signed-out visitor never sees a stuck spinner.
  const [loaded, setLoaded] = useState<{ key: string; notifs: AppNotification[] } | null>(null);
  const loadKey = user && db ? `${user.uid}:${filter}` : null;
  const notifications = loaded?.key === loadKey ? loaded.notifs : [];
  const loading = loadKey !== null && loaded?.key !== loadKey;

  useEffect(() => {
    const database = db;
    if (!user || !database || !loadKey) return;
    ...
    void (async () => {
      ...
      // Demo filler so the inbox isn't empty during demos
      if (DEMO_CONTENT_ENABLED && notifs.length === 0) {
        const demo = demoNotifications(user.uid);
        notifs = filter === 'all' ? demo : demo.filter((n) => n.type === filter);
      }

      // Records the result even on failure, so the spinner always resolves.
      if (alive) setLoaded({ key: loadKey, notifs });
    })();
